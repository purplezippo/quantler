...
backtest <- function(trade.info, cash = 0, is.tax = T, tax.rate = 0.003){
  ...
  # 3.9 alpha
  alpha <- alpha_stock(stock.price = stock.price, cash.begin = cash, trade.value = trade.value,
  ...
  # 3.11夏普比率
  sharpe.ratio <- sharpe_rate(trade.value = trade.value, rate = risk.free.rate, trade.days = strategy.days)
  ...
  if (length(strategy.type) == 2){
    ...
    win.rate.s <- win_rate(strategy.signal = strategy.s, trade.value = trade.value.s, type = 'sell',
  ...
  # 3.output ####
  all.features <-
    list(benchmark.return, strategy.return, benchmark.day.return, strategy.day.return,
  ...
  names(all.features) <-
       c("benchmark.return", "strategy.return", "benchmark.day.return", "strategy.day.return",
         "benchmark.annual.return", "strategy.annual.return",
         "beta", "alpha","alg.vol",
         "sharpe.ratio", "info.ratio", "ben.vol",
         "max.drawdown", "max.drawdown.loc",
  ...
  temp <- quantler(trade.info = trade.info, data.level = strategy.level, bt.features = all.features)
  return(temp)
}
