    ...
    if (!is.simple) {
      if (length(strategy.type) == 1) {                                                                                     # e.交易分布
        pic5 <- ggplot(temp.df4.long) +
          geom_bar(
            aes(x = date, y = value, fill = variable), width = .8,stat = "identity",
            position = "dodge"
            ) +
          geom_hline(yintercept = 0 , col = "orange", size = .5) +
          theme2 +
        ...
        pic5 <- ggplot(temp.df4.long) +
          ...
          theme2 +
          scale_fill_manual(
            values = c("sell.b" = "royalblue3", "buy.b" = "red",
      ...
      pic7 <- ggplot(temp.df6) +                                                                                          # g.收益率分布
        ...
        geom_line(aes(x = stock.time, y = -rate), col = "orange", linetype = "dashed") +
        geom_text(data = temp.df6.rate,
                  aes(x = time, y = y, label = value), size = 4, col = "orange") +
        theme2 +
        scale_color_manual(
          values = c("a.both" = "greenyellow", "b.long" = "orangered",
                     "c.short" = "royalblue1")
          )
    }
    ...
    if (is.simple) {
      if (is.pic.whole) {
        ...
        pic <- gridExtra::arrangeGrob(
          pic3, pic1, pic4, pic2, ncol = 2,layout_matrix = lay
          )
      } else {
