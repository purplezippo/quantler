...
#'    plot(test.qtler)
#' @export
setMethod('plot', signature(x = 'quantler', y = 'missing'),
  function(x, ..., plotlevel = '1day', is.simple = F, segments = F){

    if (!is.quantler(x)) stop('x must be a quantler object')
    if ( length(plotlevel) == 0 || !plotlevel %in% c('1day', '60mins', '1min')) stop('Not correct form of plot.level')
    ...
    temp.df3 <- data.frame(feature = feature.name,
                           value = round(c(x@bt.features$benchmark.return,
                                           ...
                                           x@bt.features$acc.rate.s) * 100, 2),
    ...
    if(!is.simple){                                                                   # 简化显示输出
      ...
      if(length(strategy.type) == 1){
        ...
        temp.df5.short <- plyr::ddply(temp.df5.long, .(variable), function(x){sum(x$value)})
      }else{
        # 交易分布
      ...
      if(length(strategy.type) > 1){
        ...
        temp.df6.s <- data.frame(trade.value.rate = x@bt.features$win.rate.s$pertrade.winrate,   # 每笔交易收益率
                                 stock.time = trade.info$stock.time[which(trade.info$strategy.s == -1)],   # 交易完成时间
                                 class = "c.short",rate = tax.rate, stringsAsFactors = F)
        temp.df6.b <- data.frame(trade.value.rate = x@bt.features$win.rate.b$pertrade.winrate,
        ...
        temp.df6.temp <- dplyr::full_join(temp.df6.s[,c("stock.time","trade.value.rate")],
                                   temp.df6.b[,c("stock.time","trade.value.rate")], by = "stock.time")
        ...
        temp.df6 <- data.frame(trade.value.rate = x@bt.features$win.rate.b$pertrade.winrate,
                               stock.time = trade.info$stock.time[which(trade.info$strategy.b == -1)],
                               class = 'b.buy',rate = tax.rate, stringsAsFactors = F)
      }else{
        temp.df6 <- data.frame(trade.value.rate = x@bt.features$win.rate.s$pertrade.winrate,
    ...
    if (plotlevel == "1day"){                                                                                              # a.基准收益率
      ...
      pic2 <- ggplot(temp.df1)+                                                                                            # b.回撤
        geom_hline(yintercept = cash , col = "orange", size = .5)+theme1+
        geom_line(aes(x = stock.date, y = strategy.income), col = "gold")+
        geom_point(data  = temp.df1[x@bt.features$max.drawdown.loc,],
                   aes(x = stock.date, y = strategy.income), col = "lemonchiffon", size = 2)+
        geom_line(data  = temp.df1[x@bt.features$max.drawdown.loc,],
                  aes(x = stock.date, y = strategy.income), col = "greenyellow", size = 1)+
        ggtitle(paste("Strategy return : max drawdown: ", round(x@bt.features$max.drawdown*100,2),"%",sep = ""))
    ...
    if(!is.simple){
      ...
      pic7 <- ggplot(temp.df6)+                                                                                          # g.收益率分布
        ...
        geom_segment(aes(x = stock.time,xend = stock.time, yend = 0, y = trade.value.rate , col = class), alpha = .5)+
    ...
    if (is.simple){
      ...
      if(!segments){
        ...
        gridExtra::grid.arrange(pic3, pic1, pic4, pic2, pic7, pic6, pic5, ncol = 2,layout_matrix = lay)
      }else{
